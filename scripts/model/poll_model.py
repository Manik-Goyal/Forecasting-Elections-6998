...
def model(data, polls=None):
    ...
    rho_e_bias = torch.clamp(pyro.sample("rho_e_bias", dist.Normal(0.7, 0.1)), 0, 1)
    ...
    logit_pi_democrat_state = mu_b[state, day_state] + mu_c[poll_state] + mu_m[poll_mode_state] + \
            mu_pop[poll_pop_state] + unadjusted_state* e_bias[day_state] + raw_measure_noise_state * sigma_measure_noise_state + \
